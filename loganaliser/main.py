...
class AnomalyDetection:
    ...
    def write_intermediate_metrics(log_frequency_interval, num_epochs, results_dir, results, eval_loss):
        ...
        plt.plot(this_x_axis, [x.precision for x in results], 'v-', label='Precision')
        plt.plot(this_x_axis, [x.recall for x in results], 's-', label='Recall')
        plt.plot(this_x_axis, [x.accuracy for x in results], 'P-', label='Accuracy')
        plt.xlabel('Epochs')
        plt.ylabel('Scores')
        plt.legend()
